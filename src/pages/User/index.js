...
const User = () => {
  ...
  const { data, refetch: getData, isLoading } = useQuery(['users', search, page], () => api.user({
    params: {
      page: page,
      search: search,
    }
  ...
  const deleteAction = async () => {
    const users = await del(`/users/${deleteId}`);
    setShowDelete(false)
    getData();
  }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Home" breadcrumbItem="User" />
          <Row>
            <Col xs="12">
              <Card>
                <CardBody>
