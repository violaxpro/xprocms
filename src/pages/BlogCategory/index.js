...
const Category = () => {
  ...
  const { data, refetch: getData, isLoading } = useQuery(['blog-categories', search, page], () => api.blogCategories({
    params: {
      page: page,
      search: search,
    }
  ...
  const deleteAction = async () => {
    const brands = await del(`/blog-categories/${deleteId}`);
    setShowDelete(false)
    getData();
  }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Home" breadcrumbItem="Blog Categories" />
          <Row>
            <Col xs="12">
              <Card>
                <CardBody>
