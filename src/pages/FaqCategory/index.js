...
const FaqCategory = () => {
  ...
  const { data, refetch: getData, isLoading } = useQuery(['faq-categories', search, page], () => api.faqCategory({
    params: {
      page: page,
      search: search,
    }
  ...
  const deleteAction = async () => {
    const brands = await del(`/faq-categories/${deleteId}`);
    setShowDelete(false)
    getData();
  }
  ...
  const FaqCategoryColumns = toggleModal => [
    {
      dataField: "id",
      text: "#",
      sort: true,
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Home" breadcrumbItem="Faq Categories" />
          <Row>
            <Col xs="12">
              <Card>
                <CardBody>
