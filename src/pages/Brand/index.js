...
const Brand = () => {
  ...
  const { data, refetch: getData, isLoading } = useQuery(['brands', search, page], () => api.brand({
    params: {
      page: page,
      search: search,
    }
  ...
  const deleteAction = async () => {
    const brands = await del(`/brands/${deleteId}`);
    setShowDelete(false)
    getData();
  }
  ...
  const BrandColumns = toggleModal => [
    {
      dataField: "id",
      text: "#",
      sort: true,
    ...
    },
    {
      dataField: "logo",
      text: "Logo",
      formatter: (cellContent, row, i) => <img src={getImageUrl(row.logo)} height={50} />,
    },
    {
      dataField: "website",
      text: "Website",
      formatter: (cellContent, row, i) => <a href={row.website} target="_blank" rel="noreferrer">{row.website}</a>,
    },
    {
      dataField: "action",
      isDummyField: true,
